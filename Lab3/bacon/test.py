...
def test_acted_together_tiny():
    # Simple test, an actor does not exist in the database
    actor1 = 0
    actor2 = 4724
    assert lab.acted_together(db_small, actor1, actor2) is False
...
def test_bacon_path_large():
    # Tesing the bacon path in large.pickle with Julia Roberts's ID (actor_id=1204), one valid path is [4724, 3087, 1204], 
    # showing that Kevin Bacon (4724) has acted with Robert Duvall (3087), who in turn acted with 
    # Julia Roberts (1204).
    actor_id = 1204
    len_expected = 2    
    result = lab.bacon_path(db_large, actor_id)
...
def test_actor_to_actor_path_tiny():
    # Bacon path, tiny database, from 1640 to 1532
    actor_1 = 1640
    actor_2 = 1532
    len_expected = 2
